...
public class PlayerAbility : MonoBehaviour
{
    ...
    void OnFlash()
    {
        ...
        // ✅ 콜라이더 중심(없으면 transform.position)
        var col = GetComponent<Collider>();
        Vector3 center = col ? col.bounds.center : transform.position;

        // ✅ CapsuleCast 우선, 없으면 Raycast 폴백
        bool blocked = false;
        ...
        if (col is CapsuleCollider cap)
        {
            ...
            if (Physics.CapsuleCast(p1, p2, r, flashDir, out RaycastHit hit, flashDistance, mask, QueryTriggerInteraction.Ignore))
                blocked = hit.collider.CompareTag("Platform") || hit.collider.CompareTag("Die");
        }
        else
        {
            if (Physics.Raycast(center, flashDir, out RaycastHit hit, flashDistance, mask, QueryTriggerInteraction.Ignore))
                blocked = hit.collider.CompareTag("Platform") || hit.collider.CompareTag("Die");
        }
